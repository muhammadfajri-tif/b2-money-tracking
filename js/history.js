...
function handleBackupAccount() {
  ...
  const fileType = prompt(
    "Pilih format data yang akan di download: [json/csv] (default: json)",
    "json"
  ).toString();
...
const exportIncomeUserButton = document.getElementById(
  "export-income-json-button"
);
...
function handleExportIncome() {
  ...
  const fileName = `${getUsername()}_income-data_money-tracking-by-b2.${
    metadataFile.ext
  }`;
...
const exportSpendingUserButton = document.getElementById(
  "export-spending-json-button"
);
...
function handleExportSpending() {
  ...
  // create downloadable file
  const metadataFile = exportUserData(data, "json");
  const fileName = `${getUsername()}_spending-data_money-tracking-by-b2.${
    metadataFile.ext
  }`;
...
const exportIncomeUserCSVButton = document.getElementById(
  "export-income-csv-button"
);
...
const exportSpendingUserCSVButton = document.getElementById(
  "export-spending-csv-button"
);
